...
if [[ "${PV}" == "9999" ]]; then
	...
	MOZC_GIT_REVISION="a3e19dec25cb6d1414cf3e39a418fc398ed67b49"
	MOZC_DATE="${PV#*_p}"
	MOZC_DATE="${MOZC_DATE%%_p*}"

	FCITX_MOZC_GIT_REVISION="7fc8df058c3bcee6adb845341064ba8fa8f47b1a"
	...
	ABSEIL_GIT_REVISION="e1d388e7e74803050423d035e4374131b9b57919"

	UT_DICT_VER="20210923"
fi
...
if [[ "${PV}" == "9999" ]]; then
	...
	SRC_URI="https://github.com/google/mozc/archive/${MOZC_GIT_REVISION}.tar.gz -> ${PN}-${PV%%_p*}-${MOZC_DATE}.tar.gz
		...
		ut? ( https://osdn.net/downloads/users/33/33018/mozcdic-ut-${UT_DICT_VER}.tar.bz2 -> mozcdic-ut-${UT_DICT_VER}.tar.bz2 )"
fi
